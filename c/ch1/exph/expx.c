...
int main(int argc, char **argv) {
  PetscMPIInt    rank;
  PetscInt       i, N = 100 /* total number of terms to use */;
  ...
  if (x < 0.0) {
    ...
    localval = horner_exp_terms(x, n_start, n_end - 1);
  ...
  rel_error = PetscAbsReal(globalsum - PetscExpReal((PetscReal)x)) / PetscAbsReal(PetscExpReal((PetscReal)x)) / PETSC_MACHINE_EPSILON;
